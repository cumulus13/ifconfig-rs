...
//! # Platform Support
//!
//! This tool is **Windows-only**. On other platforms, it will print
//! a compatibility message and exit.
...
// =============================================================================
// CONFIGURATION & STYLING (Cross-platform)
// =============================================================================
...
// =============================================================================
// ANSI STYLING ENGINE (Cross-platform)
// =============================================================================
...
struct Args {
    ...
    #[arg(short = 'i', long = "interface", value_name = "IFACE", help = "Specify interface name (supports partial matching)")]
    ...
    #[arg(short = 's', long = "set", num_args = 2.., value_name = "CONFIG", help = "Set IP configuration: IP NETMASK [GATEWAY] [DNS1] [DNS2] ...")]
    set: Option<Vec<String>>,

    #[arg(short = 'd', long = "dns", num_args = 1.., value_name = "DNS", help = "Set DNS servers only: DNS1 DNS2 ...")]
    dns: Option<Vec<String>>,

    #[arg(long = "dhcp", help = "Reset interface to DHCP")]
    dhcp: bool,

    #[arg(long = "restore", help = "Restore saved configuration from backup")]
    restore: bool,
...
fn config_path() -> PathBuf {
    ...
    if let Ok(json) = serde_json::to_string_pretty(&Config::default()) {
        let _ = std::fs::write(&home_path, json);
    }
...
mod platform {
    ...
    pub fn get_dns_from_registry(guid: &str) -> Vec<String> {
        let path = format!(
    ...
    pub unsafe fn get_interfaces() -> Result<Vec<Interface>> {
        ...
        if ret != ERROR_SUCCESS.0 && ret != ERROR_BUFFER_OVERFLOW.0 {
            return Err(AppError::Other(format!("Windows API error: {ret}")));
        }
    ...
    pub fn set_dns_only(iface: &str, dns: &[String], cfg: &Config) -> Result<bool> {
        println!(
            "{}",
    ...
    pub fn set_dhcp(iface: &str, backup: &Path, cfg: &Config) -> Result<bool> {
        let interfaces = unsafe { get_interfaces()? };
        let _ = save_current_config(iface, &interfaces, backup, cfg)?;
    ...
    pub fn restore_config(iface: &str, backup: &Path, cfg: &Config) -> Result<bool> {
        if !backup.exists() {
    ...
    pub fn print_as_table(interfaces: &[Interface], cfg: &Config) {
    ...
    pub fn print_as_list(interfaces: &[Interface], cfg: &Config) {
        for iface in interfaces {
    ...
    pub fn show_help_examples(cfg: &Config) {
        println!();
    ...
    pub fn require_admin() -> Result<()> {
        unsafe {
            if !IsUserAnAdmin().as_bool() {
                return Err(AppError::Other("Administrator privileges required".into()));
            }
        }
        Ok(())
    }
    ...
    pub fn copy_to_clipboard(text: &str) -> Result<()> {
        let mut cb = Clipboard::new().map_err(|e| AppError::Other(format!("Clipboard: {e}")))?;
    ...
    pub fn run(args: Args, cfg: Config, backup_path: PathBuf) {
        ...
        let interfaces = unsafe {
            match get_interfaces() {
                Ok(v) => v,
                Err(e) => {
                    eprintln!("{}", apply_style(&format!("Error reading interfaces: {e}"), &cfg.colors.error));
                    std::process::exit(1);
                }
            }
        };
        ...
        if let Some(query) = args.get_ip {
            match interfaces.iter().find(|i| i.name.to_lowercase().contains(&query.to_lowercase())) {
                Some(iface) => {
                    if let Err(e) = copy_to_clipboard(&iface.ipv4) {
                ...
                None => {
                    println!("{}", apply_style(&format!("Interface '{query}' not found!"), &cfg.colors.error));
                }
            }
            return;
        }
        ...
        if let Some(query) = args.get_gateway {
            match interfaces.iter().find(|i| i.name.to_lowercase().contains(&query.to_lowercase())) {
                Some(iface) => {
                    if let Err(e) = copy_to_clipboard(&iface.gateway) {
                        println!("{}", apply_style(&format!("Error: {e}"), &cfg.colors.error));
                    } else {
...
// =============================================================================
// PLATFORM-SPECIFIC: Non-Windows Stub
// =============================================================================
...
mod platform {
    ...
    pub fn run(_args: Args, _cfg: Config, _backup_path: PathBuf) {
        eprintln!("{}", apply_style(
            "❌ This tool is only supported on Windows.",
            &StyleDef { fg: Some("#FF0000".into()), ..Default::default() }
        ));
        eprintln!("Please run on a Windows system.");
        std::process::exit(1);
    }
}
...
fn main() {
    ...
    platform::run(args, cfg, backup_path);
}